...
void CScene::printUnderline(std::ostringstream& buffer, int line_no) const {
  auto is_curline = (_cur_point.y == line_no);

  for (int col = 0; col < 9; ++col) {
    // col and row print red
    if ((col % 3) == 0 || (line_no + 1) % 3 == 0) {
      buffer << Color::Modifier(Color::BOLD, Color::BG_DEFAULT, Color::FG_RED)
             << CORNER << Color::Modifier();
    } else {
      buffer << CORNER;
    }

    // print ^ for _cur_point
    auto third_symbol = (is_curline && _cur_point.x == col) ? ARROW : LINE;
    if ((line_no + 1) % 3 == 0) {
      buffer << Color::Modifier(Color::BOLD, Color::BG_DEFAULT, Color::FG_RED)
             << LINE << third_symbol << LINE << Color::Modifier();
    } else {
      buffer << LINE << third_symbol << LINE;
    }
  }
